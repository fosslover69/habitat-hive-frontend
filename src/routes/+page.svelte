<div class="text-font-white">
	<div class="flex flex-col items-center justify-evenly p-8 pb-20 lg:flex-row">
		<div class="mb-8 lg:w-1/3">
			<div class="text-3xl lg:text-5xl">
				Habitat Hive - your perfect destination for hassle-free house hunting!
			</div>
			<a href="#services">
				<button class="mt-4 rounded-lg bg-primary p-2 text-xl hover:bg-background-light lg:mt-8"
					>Explore More</button
				>
			</a>
		</div>
		<div>
			<img
				src="https://ik.imagekit.io/corvus/viktor-ritsvall-lhH3UMEEsGQ-unsplash_1.png?updatedAt=1680705004445"
				alt="House"
			/>
		</div>
	</div>
	<div class="bg-background-light p-16 text-center text-xl">
		<div class="flex flex-col items-center justify-center gap-8 lg:flex-row lg:gap-48">
			<div>
				<div class="mb-2 h-48 w-48 rounded-lg bg-font-white" />
				<div>Hassle Free</div>
			</div>
			<div>
				<div class="mb-2 h-48 w-48 rounded-lg bg-font-white" />
				Free of Cost
			</div>
			<div>
				<div class="mb-2 h-48 w-48 rounded-lg bg-font-white" />
				Habitability Index
			</div>
		</div>
	</div>
	<div id="services" class="my-8 text-center">
		<h1 class="mb-8 text-3xl text-primary">Our Services</h1>
		<div class="flex flex-col items-center justify-center gap-8 text-xl lg:flex-row lg:gap-56">
			<a href="/hive-finder">
				<div class="mb-2 h-56 w-56 rounded-lg bg-font-white" />
				Hive Finder
			</a>
			<a href="/queen-calculator">
				<div class="mb-2 h-56 w-56 rounded-lg bg-font-white" />
				Queen Comfort Calculator
			</a>
		</div>
	</div>
</div>
